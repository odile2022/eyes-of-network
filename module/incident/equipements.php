			...
			<div class="table-title">
				<div class="row">
					<div class="col" style="padding:10px;">
						...
						<a href="#addEmployeeModal" class="pull-right btn btn-success" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Ajout Equipement</span></a>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="modal-content">
					<form method="post" action="controlleur.php?action=save_equipement">
						<div class="modal-header">						
							...
							<h4 class="modal-title">Ajout Equipement</h4>
						</div>
						<div class="modal-body">					
							<div class="form-group">
								<label>Adresse IP</label>
								<input type="text" class="form-control" name='adresse_ip' required>
							</div>
							<div class="form-group">
								<label>SSH User</label>
								<input type="text" class="form-control" name='ssh_user' required>
							</div>
							<div class="form-group">
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<form method="post" action="controlleur.php?action=edit_equipement">
						<div class="modal-header">						
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
							<h4 class="modal-title">Mise a jour d'equippement</h4>
						</div>
						<div class="modal-body">					
...
<?php
$extraFooterTags = "<script> var dataSet =". json_encode(DB::equipement()->tableData()).";".
<<<EOF
...
function editItem(btn, id){
	...
	$('#editEmployeeModal input[name=adresse_ip]').val(item.adresse_ip);
	$('#editEmployeeModal input[name=ssh_user]').val(item.ssh_user);
